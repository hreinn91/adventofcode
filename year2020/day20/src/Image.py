...
class Image:
    ...
    def print_image(self):
        tile_frame = self.make_tile_grid()
        for row in range(0, self.image_size):
            tile_columns = tile_frame[row]
            for segment_index in range(0, self.tile_size):
                for column in range(0, self.image_size):
                    tile = tile_columns[column]
                    segment = tile.rows[segment_index]
                    print(segment)
                print('\n')
        pass
    ...
    def make_tile_grid(self):
        ...
        for tile_row in range(0, self.image_size):
            row_of_tiles = self.tiles_container[self.image_size * tile_row: self.image_size + self.image_size * tile_row]
